...
from input_data import smoothing, show_rawdata, spacing, params_units
    ...
    for i in range(Plx):
        for k in range(Ply):
            ...
            for entry in foldernames:
                ...
                im=ax[it].plot(radius, all_data[entry][plots[ind]], label=entry, color=colors[count])
...
def plot_heat(all_data, homedir, comparison, params, params_units):
    ...
    for i in range(Plx):
        for k in range(Ply):
            ...
            if plots[ind] == list(params_units['h'].keys())[4] and smoothing == True:
                ...
                for entry in foldernames:
                    ...
                    if show_rawdata == True:
                        ...
                        ax[it].plot(all_data[entry][list(params_units['h'].keys())[0]], all_data[entry][plots[ind]], color='C'+str(i), alpha=0.5)
                    
                    p, = ax[it].plot(all_data[entry][list(params_units['h'].keys())[0]][::spacing], smooth, color='C'+str(i), label = entry)
            ...
            ax[it].set_xlabel(list(params_units['h'].keys())[0], labelpad=10)
            ax[it].set_xlim((0,None))
...
def plot_2D(all_data, homedir, comparison, params, params_units):
    ...
    if comparison == False:
            ...
            for k in range(Ply):
                ...
                if plots[ind] == list(params['b'].keys())[3]:
                    ...
                    for entry in foldernames:
                        ...
                        im = ax[it].pcolormesh(lateral_cells, time, data_pure, cmap='YlOrRd')  
                    ...
                    for entry in foldernames:
                        ...
                        if plots[ind] == list(params['b'].keys())[1]:
                            ...
                            colormap = 'YlGnBu'
                            im = ax[it].pcolormesh(time, radial_cells, data_trans, cmap=colormap, norm=mpl.colors.LogNorm())
                            ...
                            im = ax[it].pcolormesh(time, radial_cells, data_trans, cmap='magma')
        ...
        for plot in plots:
            ...
            for i in range(Plx):
                for k in range(Ply):
                    ...
                    if plot == list(params['b'].keys())[3]:
                        ...
                        im = ax[it].pcolormesh(lateral_cells, time, data_pure, cmap=colormap)  
                        
                        ax[it].set_xlabel('Lateral cells', labelpad=10)
                        ax[it].set_ylabel('Time [My]', labelpad=10)
                        ...
                        ax[it].set_xlabel('Time [My]', labelpad=10)
                        ax[it].set_ylabel('Radial cells', labelpad=10)
            ...
            fig.colorbar(im, ax=ax[:], cmap = colormap, label=params_units['b'][plot])
            plt.show()
